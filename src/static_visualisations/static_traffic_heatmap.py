...
def plot_traffic_heatmap(filtered_packets, pcap_file):
    ...
    if not packets:
        ...
        plt.text(0.5, 0.5, "No packet data available",
    ...
    if time_span.total_seconds() <= 3600:  # <= 1 hour
        ...
        plt.title(f"Traffic Heatmap by Second ({start_time.strftime('%H:%M')} - {end_time.strftime('%H:%M')})")
        ...
        plt.title(f"Traffic Heatmap by Minute ({start_time.strftime('%Y-%m-%d %H:%M')} - {end_time.strftime('%H:%M')})")
        ...
        plt.title(f"Traffic Heatmap by Hour ({start_time.strftime('%Y-%m-%d')} - {end_time.strftime('%Y-%m-%d')})")

    # Add colorbar
    cbar = plt.colorbar()
    cbar.set_label('Traffic Volume (bytes)')

    # Add file name to the title
    file_name = os.path.basename(pcap_file)
    plt.suptitle(f"Traffic Heatmap - {file_name}", fontsize=16)
